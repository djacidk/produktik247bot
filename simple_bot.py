...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('cat_'):
        ...
        await query.message.edit_text("Введите количество товара:")
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработчик текстовых сообщений (ввод количества)"""
...
📅 Дата заказа: {update.message.date.strftime('%Y-%m-%d %H:%M:%S')}
...
Нажмите кнопку "Обновить статус" для проверки актуального статуса заказа.
            ...
            # Создаем клавиатуру с кнопкой "Каталог"
            main_keyboard = [['Каталог']]
            main_reply_markup = ReplyKeyboardMarkup(main_keyboard, resize_keyboard=True)
            
            try:
                await update.message.reply_text(order_text, reply_markup=reply_markup)
                # Добавляем основную клавиатуру отдельно
                await update.message.reply_text("Продолжайте делать заказы:", reply_markup=main_reply_markup)
            except TimedOut:
                # Если таймаут, отправляем упрощенное сообщение
                simple_text = f"✅ Заказ №{order_number} оформлен! Статус: {order['status']}"
                await update.message.reply_text(simple_text, reply_markup=reply_markup)
                await update.message.reply_text("Продолжайте делать заказы:", reply_markup=main_reply_markup)
                print("⚠️ Таймаут при отправке полного сообщения подтверждения")
...
def main():
    ...
    # Обработчик текстовых сообщений (включая ввод количества)
